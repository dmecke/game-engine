...
export default class Keyboard {
    ...
    onPressedAny(callback: (code: string) => void): void {
        this.anyPressedCallbacks.push(callback);
    }

    onDownAny(callback: (code: string) => void): void {
        this.anyDownCallbacks.push(callback);
    }

    onReleasedAny(callback: (code: string) => void): void {
    ...
    private onKeyDown(event: KeyboardEvent): void {
        if (!this.pressedKeys.has(event.code)) {
            ...
            this.anyPressedCallbacks.forEach(callback => callback(event.code));
        }
        this.downListeners.forEach(listener => listener.handle(event.code));
        this.anyDownCallbacks.forEach(callback => callback(event.code));
    ...
    private onKeyUp(event: KeyboardEvent): void {
        ...
        this.anyReleasedCallbacks.forEach(callback => callback(event.code));
